...
#include "ledGpio_interface.h"
#include "ledGpio_messages.h"
...
typedef struct
{
    ...
    ledGpio_DisconnectHandler_t disconnectHandler; ///< Disconnect handler for this thread
...
)
{
    ...
    LE_FATAL_IF(clientThreadPtr==NULL,
                "ledGpio_ConnectService() not called for current thread");
...
static void InitCommonData(void)
{
    ...
    _ClientDataPool = le_mem_CreatePool("ledGpio_ClientData", sizeof(_ClientData_t));

    // Allocate the client thread pool
    _ClientThreadDataPool = le_mem_CreatePool("ledGpio_ClientThreadData", sizeof(_ClientThreadData_t));
    ...
    _HandlerRefMap = le_ref_CreateMap("ledGpio_ClientHandlers", 5);
}
 ...
 */
//--------------------------------------------------------------------------------------------------
le_result_t ledGpio_TryConnectService
(
    void
)
{
...
)
{
    ...
    LE_FATAL("Component for ledGpio disconnected\n");
}
 ...
 */
//--------------------------------------------------------------------------------------------------
void ledGpio_SetServerDisconnectHandler
(
    ledGpio_DisconnectHandler_t disconnectHandler,
    void *contextPtr
)
{
    _ClientThreadData_t* clientThreadPtr = GetClientThreadDataPtr();
 ...
 */
//--------------------------------------------------------------------------------------------------
void ledGpio_DisconnectService
(
    void
)
{
 ...
 */
//--------------------------------------------------------------------------------------------------
le_result_t ledGpio_SetInput
(
    ledGpio_Polarity_t polarity
...
)
{
    ...
    _msgPtr->id = _MSGID_ledGpio_SetInput;
    ...
    LE_ASSERT(ledGpio_PackPolarity( &_msgBufPtr, &_msgBufSize,
                                                  polarity ));
 ...
 */
//--------------------------------------------------------------------------------------------------
le_result_t ledGpio_SetPushPullOutput
(
    ledGpio_Polarity_t polarity,
        ///< [IN] Active-high or active-low.
    bool value
...
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_ledGpio_SetPushPullOutput;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
 ...
 */
//--------------------------------------------------------------------------------------------------
le_result_t ledGpio_SetTriStateOutput
(
    ledGpio_Polarity_t polarity
        ///< [IN] Active-high or active-low.
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_ledGpio_SetTriStateOutput;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
 ...
 */
//--------------------------------------------------------------------------------------------------
le_result_t ledGpio_SetOpenDrainOutput
(
    ledGpio_Polarity_t polarity,
        ///< [IN] Active-high or active-low.
    bool value
...
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_ledGpio_SetOpenDrainOutput;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
 ...
 */
//--------------------------------------------------------------------------------------------------
le_result_t ledGpio_EnablePullUp
(
    void
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_ledGpio_EnablePullUp;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
 ...
 */
//--------------------------------------------------------------------------------------------------
le_result_t ledGpio_EnablePullDown
(
    void
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_ledGpio_EnablePullDown;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
 ...
 */
//--------------------------------------------------------------------------------------------------
le_result_t ledGpio_DisableResistors
(
    void
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_ledGpio_DisableResistors;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
...
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_ledGpio_Activate;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
 ...
 */
//--------------------------------------------------------------------------------------------------
le_result_t ledGpio_Deactivate
(
    void
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_ledGpio_Deactivate;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
...
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_ledGpio_SetHighZ;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
...
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_ledGpio_Read;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
...
static void _Handle_ledGpio_AddChangeEventHandler
(
    void* _reportPtr,
    void* _dataPtr
)
{
    ...
    ledGpio_ChangeCallbackFunc_t _handlerRef_ledGpio_AddChangeEventHandler = (ledGpio_ChangeCallbackFunc_t)_clientDataPtr->handlerPtr;
    ...
    if ( _handlerRef_ledGpio_AddChangeEventHandler != NULL )
    {
        _handlerRef_ledGpio_AddChangeEventHandler(state, contextPtr );
    }
    else
    {
...
//--------------------------------------------------------------------------------------------------
/**
 * Add handler function for EVENT 'ledGpio_ChangeEvent'
 *
 ...
 */
//--------------------------------------------------------------------------------------------------
ledGpio_ChangeEventHandlerRef_t ledGpio_AddChangeEventHandler
(
    ledGpio_Edge_t trigger,
        ///< [IN] Change(s) that should trigger the callback to be called.
    ledGpio_ChangeCallbackFunc_t handlerPtr,
...
)
{
    ...
    ledGpio_ChangeEventHandlerRef_t _result;
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_ledGpio_AddChangeEventHandler;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
    ...
    // Pack the input parameters
    LE_ASSERT(ledGpio_PackEdge( &_msgBufPtr, &_msgBufSize,
                                                  trigger ));
...
//--------------------------------------------------------------------------------------------------
/**
 * Remove handler function for EVENT 'ledGpio_ChangeEvent'
 */
//--------------------------------------------------------------------------------------------------
void ledGpio_RemoveChangeEventHandler
(
    ledGpio_ChangeEventHandlerRef_t handlerRef
        ///< [IN]
)
{
    le_msg_MessageRef_t _msgRef;
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_ledGpio_RemoveChangeEventHandler;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
    ...
    _UNLOCK
    handlerRef = (ledGpio_ChangeEventHandlerRef_t)clientDataPtr->handlerRef;
 ...
 */
//--------------------------------------------------------------------------------------------------
le_result_t ledGpio_SetEdgeSense
(
    ledGpio_Edge_t trigger
        ///< [IN] Change(s) that should trigger the callback to be called.
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_ledGpio_SetEdgeSense;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
 ...
 */
//--------------------------------------------------------------------------------------------------
le_result_t ledGpio_DisableEdgeSense
(
    void
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_ledGpio_DisableEdgeSense;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
...
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_ledGpio_IsOutput;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
...
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_ledGpio_IsInput;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
 ...
 */
//--------------------------------------------------------------------------------------------------
ledGpio_Edge_t ledGpio_GetEdgeSense
(
    void
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_ledGpio_GetEdgeSense;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
    ...
    // Unpack the result first
    if (!ledGpio_UnpackEdge( &_msgBufPtr, &_msgBufSize, &_result ))
    {
        goto error_unpack;
    }
 ...
 */
//--------------------------------------------------------------------------------------------------
ledGpio_Polarity_t ledGpio_GetPolarity
(
    void
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_ledGpio_GetPolarity;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
    ...
    // Unpack the result first
    if (!ledGpio_UnpackPolarity( &_msgBufPtr, &_msgBufSize, &_result ))
    {
        goto error_unpack;
    }
...
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_ledGpio_IsActive;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
 ...
 */
//--------------------------------------------------------------------------------------------------
ledGpio_PullUpDown_t ledGpio_GetPullUpDown
(
    void
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_ledGpio_GetPullUpDown;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
    ...
    // Unpack the result first
    if (!ledGpio_UnpackPullUpDown( &_msgBufPtr, &_msgBufSize, &_result ))
    {
        goto error_unpack;
    }
...
)
{
    ...
    switch (msgPtr->id)
    {
        case _MSGID_ledGpio_AddChangeEventHandler :
            le_event_QueueFunctionToThread(callersThreadRef, _Handle_ledGpio_AddChangeEventHandler, msgRef, clientDataPtr);
            break;
