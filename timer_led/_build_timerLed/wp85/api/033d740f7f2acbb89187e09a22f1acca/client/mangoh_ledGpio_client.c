...
#include "mangoh_ledGpio_interface.h"
#include "mangoh_ledGpio_messages.h"
...
typedef struct
{
    ...
    mangoh_ledGpio_DisconnectHandler_t disconnectHandler; ///< Disconnect handler for this thread
...
)
{
    ...
    LE_FATAL_IF(clientThreadPtr==NULL,
                "mangoh_ledGpio_ConnectService() not called for current thread");
...
static void InitCommonData(void)
{
    ...
    _ClientDataPool = le_mem_CreatePool("mangoh_ledGpio_ClientData", sizeof(_ClientData_t));

    // Allocate the client thread pool
    _ClientThreadDataPool = le_mem_CreatePool("mangoh_ledGpio_ClientThreadData", sizeof(_ClientThreadData_t));
    ...
    _HandlerRefMap = le_ref_CreateMap("mangoh_ledGpio_ClientHandlers", 5);
}
 ...
 */
//--------------------------------------------------------------------------------------------------
void mangoh_ledGpio_ConnectService
(
    void
)
{
 ...
 */
//--------------------------------------------------------------------------------------------------
le_result_t mangoh_ledGpio_TryConnectService
(
    void
)
{
...
)
{
    ...
    LE_FATAL("Component for mangoh_ledGpio disconnected\n");
}
 ...
 */
//--------------------------------------------------------------------------------------------------
void mangoh_ledGpio_SetServerDisconnectHandler
(
    mangoh_ledGpio_DisconnectHandler_t disconnectHandler,
    void *contextPtr
)
{
    _ClientThreadData_t* clientThreadPtr = GetClientThreadDataPtr();
 ...
 */
//--------------------------------------------------------------------------------------------------
void mangoh_ledGpio_DisconnectService
(
    void
)
{
 ...
 */
//--------------------------------------------------------------------------------------------------
le_result_t mangoh_ledGpio_SetInput
(
    mangoh_ledGpio_Polarity_t polarity
...
)
{
    ...
    _msgPtr->id = _MSGID_mangoh_ledGpio_SetInput;
    ...
    LE_ASSERT(mangoh_ledGpio_PackPolarity( &_msgBufPtr, &_msgBufSize,
                                                  polarity ));
 ...
 */
//--------------------------------------------------------------------------------------------------
le_result_t mangoh_ledGpio_SetPushPullOutput
(
    mangoh_ledGpio_Polarity_t polarity,
        ///< [IN] Active-high or active-low.
    bool value
...
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_mangoh_ledGpio_SetPushPullOutput;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
 ...
 */
//--------------------------------------------------------------------------------------------------
le_result_t mangoh_ledGpio_SetTriStateOutput
(
    mangoh_ledGpio_Polarity_t polarity
        ///< [IN] Active-high or active-low.
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_mangoh_ledGpio_SetTriStateOutput;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
 ...
 */
//--------------------------------------------------------------------------------------------------
le_result_t mangoh_ledGpio_SetOpenDrainOutput
(
    mangoh_ledGpio_Polarity_t polarity,
        ///< [IN] Active-high or active-low.
    bool value
...
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_mangoh_ledGpio_SetOpenDrainOutput;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
 ...
 */
//--------------------------------------------------------------------------------------------------
le_result_t mangoh_ledGpio_EnablePullUp
(
    void
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_mangoh_ledGpio_EnablePullUp;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
 ...
 */
//--------------------------------------------------------------------------------------------------
le_result_t mangoh_ledGpio_EnablePullDown
(
    void
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_mangoh_ledGpio_EnablePullDown;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
 ...
 */
//--------------------------------------------------------------------------------------------------
le_result_t mangoh_ledGpio_DisableResistors
(
    void
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_mangoh_ledGpio_DisableResistors;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
 ...
 */
//--------------------------------------------------------------------------------------------------
le_result_t mangoh_ledGpio_Activate
(
    void
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_mangoh_ledGpio_Activate;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
 ...
 */
//--------------------------------------------------------------------------------------------------
le_result_t mangoh_ledGpio_Deactivate
(
    void
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_mangoh_ledGpio_Deactivate;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
 ...
 */
//--------------------------------------------------------------------------------------------------
le_result_t mangoh_ledGpio_SetHighZ
(
    void
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_mangoh_ledGpio_SetHighZ;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
...
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_mangoh_ledGpio_Read;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
...
static void _Handle_mangoh_ledGpio_AddChangeEventHandler
(
    void* _reportPtr,
    void* _dataPtr
)
{
    ...
    mangoh_ledGpio_ChangeCallbackFunc_t _handlerRef_mangoh_ledGpio_AddChangeEventHandler = (mangoh_ledGpio_ChangeCallbackFunc_t)_clientDataPtr->handlerPtr;
    ...
    if ( _handlerRef_mangoh_ledGpio_AddChangeEventHandler != NULL )
    {
        _handlerRef_mangoh_ledGpio_AddChangeEventHandler(state, contextPtr );
    }
    else
    {
...
//--------------------------------------------------------------------------------------------------
/**
 * Add handler function for EVENT 'mangoh_ledGpio_ChangeEvent'
 *
 ...
 */
//--------------------------------------------------------------------------------------------------
mangoh_ledGpio_ChangeEventHandlerRef_t mangoh_ledGpio_AddChangeEventHandler
(
    mangoh_ledGpio_Edge_t trigger,
        ///< [IN] Change(s) that should trigger the callback to be called.
    mangoh_ledGpio_ChangeCallbackFunc_t handlerPtr,
...
)
{
    ...
    mangoh_ledGpio_ChangeEventHandlerRef_t _result;
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_mangoh_ledGpio_AddChangeEventHandler;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
    ...
    // Pack the input parameters
    LE_ASSERT(mangoh_ledGpio_PackEdge( &_msgBufPtr, &_msgBufSize,
                                                  trigger ));
...
//--------------------------------------------------------------------------------------------------
/**
 * Remove handler function for EVENT 'mangoh_ledGpio_ChangeEvent'
 */
//--------------------------------------------------------------------------------------------------
void mangoh_ledGpio_RemoveChangeEventHandler
(
    mangoh_ledGpio_ChangeEventHandlerRef_t handlerRef
        ///< [IN]
)
{
    le_msg_MessageRef_t _msgRef;
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_mangoh_ledGpio_RemoveChangeEventHandler;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
    ...
    _UNLOCK
    handlerRef = (mangoh_ledGpio_ChangeEventHandlerRef_t)clientDataPtr->handlerRef;
 ...
 */
//--------------------------------------------------------------------------------------------------
le_result_t mangoh_ledGpio_SetEdgeSense
(
    mangoh_ledGpio_Edge_t trigger
        ///< [IN] Change(s) that should trigger the callback to be called.
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_mangoh_ledGpio_SetEdgeSense;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
 ...
 */
//--------------------------------------------------------------------------------------------------
le_result_t mangoh_ledGpio_DisableEdgeSense
(
    void
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_mangoh_ledGpio_DisableEdgeSense;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
...
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_mangoh_ledGpio_IsOutput;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
...
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_mangoh_ledGpio_IsInput;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
 ...
 */
//--------------------------------------------------------------------------------------------------
mangoh_ledGpio_Edge_t mangoh_ledGpio_GetEdgeSense
(
    void
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_mangoh_ledGpio_GetEdgeSense;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
    ...
    // Unpack the result first
    if (!mangoh_ledGpio_UnpackEdge( &_msgBufPtr, &_msgBufSize, &_result ))
    {
        goto error_unpack;
    }
 ...
 */
//--------------------------------------------------------------------------------------------------
mangoh_ledGpio_Polarity_t mangoh_ledGpio_GetPolarity
(
    void
)
{
    ...
    mangoh_ledGpio_Polarity_t _result;
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_mangoh_ledGpio_GetPolarity;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
    ...
    // Unpack the result first
    if (!mangoh_ledGpio_UnpackPolarity( &_msgBufPtr, &_msgBufSize, &_result ))
    {
        goto error_unpack;
    }
...
)
{
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_mangoh_ledGpio_IsActive;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
 ...
 */
//--------------------------------------------------------------------------------------------------
mangoh_ledGpio_PullUpDown_t mangoh_ledGpio_GetPullUpDown
(
    void
)
{
    ...
    mangoh_ledGpio_PullUpDown_t _result;
    ...
    _msgRef = le_msg_CreateMsg(GetCurrentSessionRef());
    _msgPtr = le_msg_GetPayloadPtr(_msgRef);
    _msgPtr->id = _MSGID_mangoh_ledGpio_GetPullUpDown;
    _msgBufPtr = _msgPtr->buffer;
    _msgBufSize = _MAX_MSG_SIZE;
    ...
    // Unpack the result first
    if (!mangoh_ledGpio_UnpackPullUpDown( &_msgBufPtr, &_msgBufSize, &_result ))
    {
        goto error_unpack;
    }
...
)
{
    ...
    switch (msgPtr->id)
    {
        case _MSGID_mangoh_ledGpio_AddChangeEventHandler :
            le_event_QueueFunctionToThread(callersThreadRef, _Handle_mangoh_ledGpio_AddChangeEventHandler, msgRef, clientDataPtr);
            break;
